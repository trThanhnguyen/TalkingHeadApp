...
def train(net, epoch, batch_size=16, lr=0.001):
    ...
    for e in range(epoch):
        net.train()
        with tqdm(total=len(train_dataset), desc=f'Epoch {e}/{epoch}', unit='img') as p:
            running_loss = 0.0
        ...
        if (e != 0) and (e % args.eval_interval == 0):
            val_loss = evaluate(net, val_loader, criterion, content_loss, syncnet)
            val_eps.append(e)
            val_losses.append(val_loss)
            print(f"[Epoch {e}] Validation loss: {val_loss:.6f}")
    ...
    plt.plot(train_eps, train_losses, color='blue', label='train')
    plt.plot(val_eps, val_losses, color='orange', label='valid')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.yscale('log')
