...
def train(opts):
    ...
    if opts.resume is not None:
        ckpt_path = os.path.join(opts.save_dir, f"{opts.resume}.pth")
        print(f"Resuming from checkpoint: {ckpt_path}")
        model.load_state_dict(torch.load(ckpt_path))
        start_epoch = opts.resume + 1
    ...
    plt.plot(eps, train_losses, color='blue', label='train')
    plt.plot(eps, val_losses, color='orange', label='valid')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.yscale('log')
    plt.title('Training and validation losses')
    plt.tight_layout()
    plt.savefig(os.path.join(opts.save_dir, 'learning_curve.png'))
...
if __name__ == "__main__":
    ...
    p.add_argument('--asr',          choices=['wenet', 'hubert'], default='hubert')
    ...
    p.add_argument('--resume',       type=int, default=None,
                   help="Epoch number of checkpoint to resume from (e.g., 11 for 11.pth)")
    opts = p.parse_args()
