...
class BaseEncoder(torch.nn.Module):
    ...
    def forward_chunk_by_chunk(
        ...
        #print('chunk by chunk input size:', xs.size())
        #assert 0==1
        ...
        #print('stride = subsampling * decoding_chunk_size:', stride)
        #assert 0==1
        decoding_window = (decoding_chunk_size - 1) * subsampling + context
        #print('decoding_window:',decoding_window)
        ...
        #print('decoding_chunk_size:',decoding_chunk_size, 'num_decoding_left_chunks:',num_decoding_left_chunks, 'required_cache_size:', required_cache_size)
        ...
        for cur in range(0, num_frames - context + 1, stride):
            ...
            chunk_xs = xs[:, cur:end, :]
            #print('chunk_xs size: ',chunk_xs.size()) #[1, 67, 80] [1, 67, 80]...[1, 54, 80]
            #根据预设的16大小的chunksize，推算出在帧层面的划窗大小为67
            (y, subsampling_cache, elayers_output_cache,
             conformer_cnn_cache) = self.forward_chunk(chunk_xs, offset,
                                                       ...
                                                       conformer_cnn_cache)
            #print('encoder output size:', y.shape) #[1, 16, 256] [1, 16, 256]...[1, 12 256]
            #print(len(subsampling_cache[0])) #, elayers_output_cache.shape, conformer_cnn_cache.shape)
            #assert 0==1
            outputs.append(y)
            offset += y.size(1)
        ...
        #print(ys.shape) #[1, 124, 256]
        #assert 0==1
        masks = torch.ones(1, ys.size(1), device=ys.device, dtype=torch.bool)
        #ones应该是全可以看到，没有mask，而zeros则是全不可见，全部mask
        #masks = torch.zeros(1, ys.size(1), device=ys.device, dtype=torch.bool)
        masks = masks.unsqueeze(1)
        return ys, masks
